...
- **认证**: [harbor-services](https://harbor.qianting.xyz)（Google OAuth）+ localStorage 双 token（access / refresh）+ UserManager
...
2. **认证**：Google OAuth（harbor authorize → 回调 `/login/callback` 换 token）→ localStorage + UserManager；应用入口由 `HttpClientInit` 恢复 token 并 `initHttpClient({ getToken })`；认证 API 使用 `baseURL: HARBOR_BASE`。
