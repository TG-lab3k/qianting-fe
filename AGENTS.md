...
- **HTTP**: 统一 [src/core/http-client](src/core/http-client/index.ts)（Axios 单例、Bearer 注入、`Result<T>` 封装）
...
| **src/core/** | 与框架解耦的核心：http-client、auth、user、firebase |
...
2. **认证**：Firebase 登录 → 后端换 access_token → sessionStorage + UserManager；应用入口由 `HttpClientInit` 恢复 token 并 `initHttpClient({ getToken })`。
...
- **Core**: [src/core/http-client/index.ts](src/core/http-client/index.ts)、[src/core/auth](src/core/auth)、[src/core/user](src/core/user)、[src/core/firebase.ts](src/core/firebase.ts)
